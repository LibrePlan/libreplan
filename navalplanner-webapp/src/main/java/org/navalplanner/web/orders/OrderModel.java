...
public class OrderModel implements IOrderModel {
    ...
    private Map<OrderElement, String> oldOrderCodeChildren = new HashMap<OrderElement, String>();

    private OrderElementTreeModel orderElementTreeModel;

    private Boolean generateCodeOldOrder;
    ...
    private void initOldCodes() {
        if (order != null) {
            setOldOrderCode(order.getCode());
            for (OrderElement orderElement : order.getAllOrderElements()) {
                oldOrderCodeChildren.put(orderElement, orderElement.getCode());
            }
            setGenerateCodeOldOrder(order.isCodeAutogenerated());
        }
    }

    private void restoreOldCodes() {
        order.setCode(getOldOrderCode());
        for (OrderElement orderElement : oldOrderCodeChildren.keySet()) {
            orderElement.setCode(oldOrderCodeChildren.get(orderElement));
        }
    }
    ...
    private void setOldOrderCode(String oldOrderCode) {
        this.oldOrderCode = oldOrderCode;
    }

    private String getOldOrderCode() {
        return oldOrderCode;
    }

    public void setOldOrderCodeChildren(
            Map<OrderElement, String> oldOrderCodeChildren) {
        this.oldOrderCodeChildren = oldOrderCodeChildren;
    }

    public Map<OrderElement, String> getOldOrderCodeChildren() {
        return oldOrderCodeChildren;
    }

    private void setGenerateCodeOldOrder(Boolean generatedCode) {
        this.generateCodeOldOrder = generatedCode;
    }

    private Boolean isGenerateCodeOldOrder() {
        return (generateCodeOldOrder != null) ? generateCodeOldOrder : false;
    }
}
