...
public class WorkReportModel implements IWorkReportModel {
    ...
    private String oldCodeWorkReport;

    private Boolean oldGeneratedCode = false;

    private Map<WorkReportLine,String> oldCodesWorkReportLines = new HashMap<WorkReportLine,String>();
    ...
    private void generateWorkReportLineCodes() {
        ...
        try {
            EntitySequence entitySequence = entitySequenceDAO
                    .getActiveEntitySequence(EntityNameEnum.WORK_REPORT);
        ...
        workReport.generateWorkReportLineCodes(numberOfDigits);
    }
    ...
    private void initOldCodes() {
        if (workReport != null) {
            setOldCodeWorkReport(workReport.getCode());
            for (WorkReportLine line : this.workReport.getWorkReportLines()) {
                oldCodesWorkReportLines.put(line, line.getCode());
            }
            this.setOldGeneratedCode(workReport.getGenerateCode());
        }
    }

    private void restoreOldCodes() {
        workReport.setCode(getOldCodeWorkReport());
        for (WorkReportLine workReportLine : oldCodesWorkReportLines.keySet()) {
            workReportLine.setCode(oldCodesWorkReportLines.get(workReportLine));
        }
    }
    ...
    public void setCodeAutogenerated(boolean codeAutogenerated)
        ...
        if (workReport != null) {
            ...
            workReport.setGenerateCode(codeAutogenerated);
        }
    }

    private void setDefaultWorkReportCode()
            throws ConcurrentModificationException {
        String code = entitySequenceDAO
                .getNextEntityCode(EntityNameEnum.WORK_REPORT);
        if (code == null) {
            throw new ConcurrentModificationException(
                    _("Could not get work report code, please try again later"));
        }
        this.workReport.setCode(code);
    }

    public void setOldCodeWorkReport(String oldCodeWorkReport) {
        this.oldCodeWorkReport = oldCodeWorkReport;
    }

    public String getOldCodeWorkReport() {
        return oldCodeWorkReport;
    }

    public void setOldGeneratedCode(Boolean oldGeneratedCode) {
        this.oldGeneratedCode = oldGeneratedCode;
    }

    public Boolean isOldGeneratedCode() {
        return oldGeneratedCode;
    }
