...
public class MachineModel implements IMachineModel {
    ...
    private String oldCodeMachine;
    private Boolean generateCodeOldMachine;
    ...
    public void initCreate() {
        ...
        machine.setGenerateCode(configurationDAO.getConfiguration()
                .getGenerateCodeForResources());
        if (machine.getGenerateCode()) {
            setDefaultMachineCode();
        }
    }
    ...
    private void setDefaultMachineCode() throws ConcurrentModificationException {
        String code = entitySequenceDAO
                .getNextEntityCode(EntityNameEnum.MACHINE);
        if (code == null) {
            throw new ConcurrentModificationException(
                    _("Could not get machine code, please try again later"));
        }
        this.machine.setCode(code);
    }
    ...
    private void initOldCodes() {
        if (machine != null) {
            setOldCodeMachine(machine.getCode());
            setGenerateCodeOldMachine(machine.getGenerateCode());
        }
    }

    private void restoreOldCodes() {
        machine.setCode(getOldCodeMachine());
    }
    ...
    private void setOldCodeMachine(String oldCodeMachine) {
        this.oldCodeMachine = oldCodeMachine;
    }

    private String getOldCodeMachine() {
        return oldCodeMachine;
    }

    private void setGenerateCodeOldMachine(Boolean generatedCode) {
        this.generateCodeOldMachine = generatedCode;
    }

    private Boolean isGenerateCodeOldMachine() {
        return (generateCodeOldMachine != null) ? generateCodeOldMachine
                : false;
    }
}
