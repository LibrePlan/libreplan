...
public class MaterialsModel implements IMaterialsModel {
    ...
    private void initializeMaterialCategories() {
        ...
        for (MaterialCategory materialCategory: categories) {
            ...
            if (materialCategory.getGenerateCode()) {
    ...
    public void addMaterialCategory(MaterialCategory parent, String categoryName) throws ValidationException {
        ...
        MaterialCategory child = MaterialCategory.createUnvalidated("",
                _(categoryName));
        if (generateCode) {
            this.setDefaultMaterialCategoryCode(child);
        }
    ...
    private void restoreOldCodes(MaterialCategory materialCategory) {
        materialCategory.setCode(oldCodes.get(materialCategory));
    }
    ...
    public void setCodeAutogenerated(boolean codeAutogenerated,
        ...
        if (materialCategory != null) {
            if (codeAutogenerated) {
                if (materialCategory.isNewObject()) {
                    setDefaultMaterialCategoryCode(materialCategory);
                } else {
                    if (isGenerateCodeOldMaterialCategory(materialCategory)) {
                        restoreOldCodes(materialCategory);
                    } else {
                        setDefaultMaterialCategoryCode(materialCategory);
                    }
                }
            }
            materialCategory.setGenerateCode(codeAutogenerated);
        }
    }

    private void setDefaultMaterialCategoryCode(
            MaterialCategory materialCategory)
            throws ConcurrentModificationException {
        String code = entitySequenceDAO
                .getNextEntityCode(EntityNameEnum.MATERIAL_CATEGORY);
        if (code == null) {
            throw new ConcurrentModificationException(
                    _("Could not get code of the material category , please try again later"));
        }
        materialCategory.setCode(code);
    }

    private boolean isGenerateCodeOldMaterialCategory(
            MaterialCategory materialCategory) {
        return oldCodes.get(materialCategory) != null;
    }
