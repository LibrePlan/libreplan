...
public class BaseCalendarModel implements IBaseCalendarModel {
    ...
    private String oldCodeCalendar;

    private Boolean generateCodeOldCalendar;
    ...
    public void initCreate() {
        ...
        if (codeGenerated) {
            setDefaultBaseCalendarCode();
        }
        baseCalendar.setGeneratedCode(codeGenerated);
    }
    ...
    private void initOldCodes() {
        if (baseCalendar != null) {
            setOldCodeCalendar(baseCalendar.getCode());
            setGenerateCodeOldCalendar(baseCalendar.isGeneratedCode());
        }
    }

    private void restoreOldCodes() {
        baseCalendar.setCode(getOldCodeCalendar());
    }
    ...
    public void setCodeAutogenerated(boolean codeAutogenerated)
        ...
        if (baseCalendar != null) {
            if (codeAutogenerated) {
                if (baseCalendar.isNewObject()) {
                    setDefaultBaseCalendarCode();
                } else {
                    if (isGenerateCodeOldCalendar()) {
                        restoreOldCodes();
                    } else {
                        setDefaultBaseCalendarCode();
                    }
            ...
            baseCalendar.setGeneratedCode(codeAutogenerated);
        }
    }

    private void setDefaultBaseCalendarCode()
            throws ConcurrentModificationException {
        String code = entitySequenceDAO
                .getNextEntityCode(EntityNameEnum.CALENDAR);
        if (code == null) {
            throw new ConcurrentModificationException(
                    _("Could not get base calendar code, please try again later"));
        }
        this.baseCalendar.setCode(code);
    }

    public void setOldCodeCalendar(String oldCodeCalendar) {
        this.oldCodeCalendar = oldCodeCalendar;
    }

    public String getOldCodeCalendar() {
        return oldCodeCalendar;
    }

    public void setGenerateCodeOldCalendar(Boolean generateCodeOldCalendar) {
        this.generateCodeOldCalendar = generateCodeOldCalendar;
    }

    public Boolean isGenerateCodeOldCalendar() {
        return generateCodeOldCalendar;
    }
