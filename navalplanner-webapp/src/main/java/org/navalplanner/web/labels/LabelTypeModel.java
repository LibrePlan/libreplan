...
public class LabelTypeModel implements ILabelTypeModel {
    ...
    private String oldCodeLabelType;

    private Boolean generateCodeOldLabelType;

    private Map<Label, String> oldLabelCodeChildren = new HashMap<Label, String>();
    ...
    public void setCodeAutogenerated(boolean codeAutogenerated)
        ...
        if (labelType != null) {
            ...
            labelType.setGenerateCode(codeAutogenerated);
        }
    }

    private void setDefaultLabelTypeCode()
            throws ConcurrentModificationException {
        String code = entitySequenceDAO.getNextEntityCode(EntityNameEnum.LABEL);
        if (code == null) {
            throw new ConcurrentModificationException(
                    _("Could not get label type code, please try again later"));
        }
        this.labelType.setCode(code);
    }
    ...
    private void generateLabelCodes() {
        ...
        try {
            EntitySequence entitySequence = entitySequenceDAO
                    .getActiveEntitySequence(EntityNameEnum.LABEL);
        ...
        labelType.generateLabelCodes(numberOfDigits);
    }
    ...
    private void initOldCodes() {
        if (labelType != null) {
            setOldCodeLabelType(labelType.getCode());
            for (Label label : labelType.getLabels()) {
                oldLabelCodeChildren.put(label, label.getCode());
            }
            setGenerateCodeOldLabelType(labelType.getGenerateCode());
        }
    }

    private void restoreOldCodes() {
        labelType.setCode(getOldCodeLabelType());
        for (Label label : oldLabelCodeChildren.keySet()) {
            label.setCode(oldLabelCodeChildren.get(label));
        }
    }
    ...
    private void setOldCodeLabelType(String oldCodeLabelType) {
        this.oldCodeLabelType = oldCodeLabelType;
    }

    private String getOldCodeLabelType() {
        return oldCodeLabelType;
    }

    private void setGenerateCodeOldLabelType(Boolean generateCodeOldLabelType) {
        this.generateCodeOldLabelType = generateCodeOldLabelType;
    }

    private Boolean isGenerateCodeOldLabelType() {
        return generateCodeOldLabelType != null ? generateCodeOldLabelType
                : false;
    }
