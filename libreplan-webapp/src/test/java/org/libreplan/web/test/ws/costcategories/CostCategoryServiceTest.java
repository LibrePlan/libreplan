 ...
 *
 * ATTENTION: this test class should be executed with all test methods together!
 * Because there is method createAPairTypeOfWorkHours() that creates needed for another tests entities.
 * And it is separate test method with Rollback annotation.
 *
 * Every single test except createAPairTypeOfWorkHours() is not atomic!
 *
 *
...
public class CostCategoryServiceTest {
    ...
    /*
    If createAPairTypeOfWorkHours() executed first, then result - success.
    Order varies from time to time (without ordered methods)
    Correct order:
    1. testA
    2. testB
    3. testC
    */
    ...
    @Test
    @Rollback(false)
    public void testA_CreateAPairTypeOfWorkHours() {
        givenTypeOfWorkHours(typeOfWorkHoursCodeA);
        givenTypeOfWorkHours(typeOfWorkHoursCodeB);
    }

    @Test
    public void testB_AddAndGetCostCategories() {

        // Build cost category (5 constraint violations)

        // Missing cost category name and enabled.
        CostCategoryDTO cc1 = new CostCategoryDTO(null, true, new HashSet<HourCostDTO>());

        // Valid cost category DTO without hour cost
        CostCategoryDTO cc2 = new CostCategoryDTO("cc2", true, new HashSet<HourCostDTO>());
        ...
        HourCostDTO cc3_1_HourCostDTO = new HourCostDTO(new BigDecimal(3), initDate, endDate, typeOfWorkHoursCodeA);
        cc3_HourCostDTOs.add(cc3_1_HourCostDTO);
        CostCategoryDTO cc3 = new CostCategoryDTO("cc3", true, cc3_HourCostDTOs);

        // Valid cost category DTO with a invalid hour cost missing price cost, initDate and type
        ...
        List<InstanceConstraintViolationsDTO> instanceConstraintViolationsList =
                costCategoryService.addCostCategories(costCategoryListDTO).instanceConstraintViolationsList;

        assertTrue(instanceConstraintViolationsList.toString(), instanceConstraintViolationsList.size() == 2);

        // cc1 constraint violations
        assertTrue(
                instanceConstraintViolationsList.get(0).constraintViolations.toString(),
                instanceConstraintViolationsList.get(0).constraintViolations.size() == 1);

        // cc1 constraint violations
        assertTrue(
                instanceConstraintViolationsList.get(1).constraintViolations.toString(),
                instanceConstraintViolationsList.get(1).constraintViolations.size() == 3);
    ...
    @Test
    @NotTransactional
    public void testC_UpdateCostCategory() throws InstanceNotFoundException {
        ...
        final HourCostDTO cc1_1_HourCostDTO =
                new HourCostDTO(hourCostCode, new BigDecimal(3), initDate, endDate, typeOfWorkHoursCodeA);
        ...
        final HourCost hourCost = transactionService.runOnTransaction(new IOnTransaction<HourCost>() {
            ...
            public HourCost execute() {
                try {
                    HourCost cost = hourCostDAO.findByCode(hourCostCode);
    ...
    private CostCategoryListDTO createCostCategoryListDTO(CostCategoryDTO... costCategories) {
        ...
        Collections.addAll(costCategoryList, costCategories);
