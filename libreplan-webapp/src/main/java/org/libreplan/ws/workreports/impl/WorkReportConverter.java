...
final class WorkReportConverter {
    ...
    public static WorkReport toEntity(WorkReportDTO workReportDTO) throws InstanceNotFoundException {
        ...
        bindingStrategy.assignOrderElementsToWorkReportLine(
                workReport, bindingStrategy.getOrderElementsBound(workReportDTO));

        if ( workReportDTO.resource != null ) {
            try {
                Resource resource = Registry.getResourceDAO().findByCode(workReportDTO.resource);
                workReport.setResource(resource);
            } catch (InstanceNotFoundException e) {
                workReport.setResource(null);
                throw new ValidationException("There is no resource with this code");
            }
        }
        ...
        if ( workReportDTO.labels != null && !workReportDTO.labels.isEmpty() ) {
            try {
                ...
                throw new ValidationException(
                        MessageFormat.format("There is no label with this code ", (String) e.getKey()));
            }
        }

        if ( workReportDTO.descriptionValues != null && !workReportDTO.descriptionValues.isEmpty() ) {
            workReport.setDescriptionValues(toEntity(workReportDTO.descriptionValues));
        }

        return workReport;
    }

    private static WorkReportLine toEntity(WorkReportLineDTO workReportLineDTO, WorkReport workReport)
        ...
        workReportLine.setEffort(EffortDuration.parseFromFormattedString(workReportLineDTO.numHours));

        updateTheTypeOfWorkHours(workReportLineDTO, workReportLine);

        // Optional fields
        if ( workReportLineDTO.date != null ) {
            workReportLine.setDate(DateConverter.toDate(workReportLineDTO.date));
        }

        bindingStrategy.assignOrderElementsToWorkReportLine(
                workReportLine, bindingStrategy.getOrderElementsBound(workReportLineDTO));

        updateTheResourceWithWorkReportLine(workReportLineDTO.resource, workReportLine);

        if ( workReportLineDTO.clockStart != null ) {
            workReportLine.setClockStart(DateConverter.toLocalTime(workReportLineDTO.clockStart));
        }
        if ( workReportLineDTO.clockFinish != null ) {
            workReportLine.setClockFinish(DateConverter.toLocalTime(workReportLineDTO.clockFinish));
        }

        if ( workReportLineDTO.labels != null && !workReportLineDTO.labels.isEmpty() ) {
            workReportLine.setLabels(LabelReferenceConverter.toEntity(workReportLineDTO.labels));
        }

        if ( workReportLineDTO.descriptionValues != null ) {
            workReportLine.setDescriptionValues(toEntity(workReportLineDTO.descriptionValues));
        }
    ...
    private static Set<DescriptionValue> toEntity(Set<DescriptionValueDTO> descriptionValues) {
    ...
    private static DescriptionValue toEntity(DescriptionValueDTO descriptionValueDTO) {
        return DescriptionValue.create(descriptionValueDTO.fieldName, descriptionValueDTO.value);
    }

    public static WorkReportDTO toDTO(WorkReport workReport) {

        String code = workReport.getCode();

        String workReportTypeCode;
        if ( workReport.getWorkReportType() != null ) {
            workReportTypeCode = workReport.getWorkReportType().getCode();
        } else {
            throw new ValidationException("missing work report code in a work report");
        }
        ...
        String orderElementCode = bindingStrategy.getOrderElementCodesBound(workReport);
        ...
        Set<LabelReferenceDTO> labelDTOs = LabelReferenceConverter.toDTO(workReport.getLabels());
        if ( labelDTOs.isEmpty() ) {
            labelDTOs = null;
        }

        Set<DescriptionValueDTO> descriptionValuesDTOs = toDTO(workReport.getDescriptionValues());
        ...
        return new WorkReportDTO(
                ...
                orderElementCode,
                labelDTOs,
                descriptionValuesDTOs,
                workReportLineDTOs);

    }

    public static WorkReportLineDTO toDTO(WorkReportLine line) {
        String code = line.getCode();
        XMLGregorianCalendar date = DateConverter.toXMLGregorianCalendar(line.getDate());
        ...
        if ( line.getClockFinish() != null ) {
            clockFinish = DateConverter.toXMLGregorianCalendar(line.getClockFinish());
        }
        ...
        Set<LabelReferenceDTO> labelDTOs = LabelReferenceConverter.toDTO(line.getLabels());
        if ( labelDTOs.isEmpty() ) {
            labelDTOs = null;
        }

        Set<DescriptionValueDTO> descriptionValuesDTOs = toDTO(line.getDescriptionValues());
        if ( descriptionValuesDTOs.isEmpty() ) {
            descriptionValuesDTOs = null;
        }

        return new WorkReportLineDTO(
                code,
                date,
                resource,
                orderElement,
    ...
    private static Set<DescriptionValueDTO> toDTO(Set<DescriptionValue> descriptionValues) {
    ...
    private static DescriptionValueDTO toDTO(DescriptionValue descriptionValue) {
        return new DescriptionValueDTO(descriptionValue.getFieldName(), descriptionValue.getValue());
    }


    static void updateWorkReport(WorkReport workReport, WorkReportDTO workReportDTO) throws ValidationException {

        if ( StringUtils.isBlank(workReportDTO.code) ) {
            throw new ValidationException("missing code in a work report.");
        }

        /* 1: Update the existing work report line or add new work report line */
        for (WorkReportLineDTO lineDTO : workReportDTO.workReportLines) {
            ...
            if ( StringUtils.isBlank(lineDTO.code) ) {
                throw new ValidationException("missing code in a work report line");
            }

            try {
                WorkReportLine line = workReport.getWorkReportLineByCode(lineDTO.code);
                ...
                try {
                    ...
                    throw new ValidationException("missing type of work hours in a work report line");
                }
            }
        }
        ...
        if ( workReportDTO.labels != null ) {
            for (LabelReferenceDTO labelDTO : workReportDTO.labels) {
                ...
                try {
                    ...
                    throw new ValidationException("work report has not this label type assigned");
                }
            }
        }
        ...
        if ( workReportDTO.descriptionValues != null ) {
            for (DescriptionValueDTO valueDTO : workReportDTO.descriptionValues) {
                ...
                if ( StringUtils.isBlank(valueDTO.fieldName) ) {
                    throw new ValidationException("missing field name in a description value");
                }

                try {
                    DescriptionValue value = workReport.getDescriptionValueByFieldName(valueDTO.fieldName);
                    value.setValue(StringUtils.trim(valueDTO.value));
                } catch (InstanceNotFoundException e) {
                    throw new ValidationException("work report has not any description value with this field name");
                }
            }
        }
        ...
        if ( (orderElementCode != null) && (!orderElementCode.isEmpty()) ) {
            try {
                OrderElement orderElement = Registry.getOrderElementDAO().findUniqueByCode(orderElementCode);
    ...
    private static void updateWorkReportLine(WorkReportLine workReportLine, WorkReportLineDTO workReportLineDTO)
        ...
        if ( workReportLineDTO.labels != null ) {
                ...
                try {
                    ...
                    throw new ValidationException("there are not work report lines with assigned labels of this type");
                }
            }
        }

        /* 2: Update the existing description values */
        updateDescriptionValues(workReportLineDTO.descriptionValues, workReportLine);
        ...
        /* Step 3.4: Update the clock start and the clock end and the number of hours */
        if ( workReportLineDTO.clockStart != null ) {
            workReportLine.setClockStart(DateConverter.toLocalTime(workReportLineDTO.clockStart));
        }
        if ( workReportLineDTO.clockFinish != null ) {
    ...
    private static void updateTheResourceWithWorkReportLine(String resourceCode, WorkReportLine workReportLine) {
    ...
    private static void updateTheTypeOfWorkHours(WorkReportLineDTO workReportLineDTO, WorkReportLine workReportLine) {
        if ( workReportLineDTO.typeOfWorkHours != null ) {
            try{
                TypeOfWorkHours typeOfWorkHours =
                        Registry.getTypeOfWorkHoursDAO().findUniqueByCode(workReportLineDTO.typeOfWorkHours);
    ...
    private static void updateDescriptionValues(
            Set<DescriptionValueDTO> descriptionValues, WorkReportLine workReportLine) {
        ...
        if ( descriptionValues != null ) {
            for (DescriptionValueDTO valueDTO : descriptionValues) {
                ...
                try {
                    DescriptionValue value = workReportLine.getDescriptionValueByFieldName(valueDTO.fieldName);
                    value.setValue(StringUtils.trim(valueDTO.value));
                } catch (InstanceNotFoundException e) {
                    throw new ValidationException("work report have not any description value with this field name");
                }
            }
        }
    }
    ...
    @Transactional
    private static void updateLabel(LabelReferenceDTO labelDTO, Set<Label> labels) throws InstanceNotFoundException {
        ...
        if ( !labelToAdd.getCode().equals(labelToChange.getCode()) ) {
            labels.remove(labelToChange);
            labels.add(labelToAdd);
        }
    }

    private static Label getLabelByLabelType(Set<Label> labels, LabelType type) throws InstanceNotFoundException {
