...
public class WorkerModel extends IntegrationEntityModel implements IWorkerModel {
    ...
    private final ICriterionType<?>[] laboralRelatedTypes = {
            ...
            PredefinedCriterionTypes.CATEGORY,
            PredefinedCriterionTypes.SKILL };
    ...
    private List<Worker> currentWorkerList = new ArrayList<>();
    ...
    private static void forceLoadSatisfactions(Resource resource) {
        for (CriterionSatisfaction criterionSatisfaction : resource.getAllSatisfactions()) {
    ...
    @Override
    @Transactional(readOnly = true)
    public void unassignSatisfactions(Collection<? extends CriterionSatisfaction> satisfactions) {
    ...
    private static class NullAssigner implements IMultipleCriterionActiveAssigner {

        private List<CriterionSatisfaction> empty = Collections.emptyList();

        private List<Criterion> emptyCriterions = Collections.emptyList();
        ...
        @Override
        public void unassign(Collection<? extends CriterionSatisfaction> satisfactions) {
        }
    ...
    private static class MultipleCriterionActiveAssigner implements IMultipleCriterionActiveAssigner {
        ...
        private Map<Criterion, CriterionSatisfaction> unassigned = new HashMap<>();

        private Set<CriterionSatisfaction> added = new HashSet<>();
        ...
        private List<CriterionSatisfaction> calculateInitialHistory() {
            Collection<CriterionSatisfaction> allSatisfactions = resource.getSatisfactionsFor(type);

            ArrayList<CriterionSatisfaction> result = new ArrayList<>();
        ...
        private HashSet<CriterionSatisfaction> calculateInitialActive() {
            return new HashSet<>(resource.getCurrentSatisfactionsFor(type));
        }

        private List<Criterion> calculateInitialCriterionsNotAssigned() {
            Map<Long, Criterion> allCriterions = byId(criterionDAO.findByType(type));
        ...
        public List<CriterionSatisfaction> getActiveSatisfactions() {
            Set<CriterionSatisfaction> result = new HashSet<>(added);

            for (CriterionSatisfaction criterionSatisfaction : initialActive) {
                if (!unassigned.containsKey(criterionSatisfaction.getCriterion())) {
                    result.add(criterionSatisfaction);
                }
            }
        ...
        public List<Criterion> getCriterionsNotAssigned() {
            return new ArrayList<>(unassigned.keySet());
        }

        public void unassign(Collection<? extends CriterionSatisfaction> satisfactions) {
            for (CriterionSatisfaction criterionSatisfaction : satisfactions) {
                unassigned.put(criterionSatisfaction.getCriterion(), criterionSatisfaction);
        ...
        private CriterionSatisfaction createSatisfactionFor(Criterion criterion) {
            return CriterionSatisfaction.create(new LocalDate(), criterion, resource);
        }
        ...
        public void applyChanges() {
            for (CriterionSatisfaction criterionSatisfaction : added) {
                ...
                resource.addSatisfaction(
                        new CriterionWithItsType(type, criterionSatisfaction.getCriterion()),
                        Interval.from(criterionSatisfaction.getStartDate()));
            }
    ...
    public IMultipleCriterionActiveAssigner getLocalizationsAssigner() {
        return localizationsAssigner != null ? localizationsAssigner : new NullAssigner();
    }
    ...
    private static Map<Long, Criterion> byId(Collection<? extends Criterion> criterions) {
        Map<Long, Criterion> result = new HashMap<>();
        for (Criterion criterion : criterions) {
    ...
    public Map<ICriterionType<?>, Collection<Criterion>> getLaboralRelatedCriterions() {
        Map<ICriterionType<?>, Collection<Criterion>> result = new HashMap<>();
    ...
    public List<Worker> getFilteredWorker(ResourcePredicate predicate) {
        List<Worker> filteredResourceList = new ArrayList<>();
