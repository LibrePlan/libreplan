...
public class ExpenseSheetModel extends IntegrationEntityModel implements IExpenseSheetModel {
    ...
    @Override
    @Transactional
    public void generateExpenseSheetLinesIfIsNecessary() {
    ...
    private ExpenseSheet getFromDB(ExpenseSheet expenseSheet) {
        return getFromDB(expenseSheet.getId());
    }

    @Transactional(readOnly = true)
    private ExpenseSheet getFromDB(Long id) {
        try {
            ExpenseSheet result = expenseSheetDAO.find(id);
            forceLoadExpenseSheetData(result);
            return result;
